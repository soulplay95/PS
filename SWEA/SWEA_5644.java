...
public class SWEA_5644 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int tc = 1; tc <= TC; tc++) {
			...
			pathA = new int[M + 1]; // 사용자 A의 궤적
			pathB = new int[M + 1]; // 사용자 B의 궤적
			bc = new int[bcCnt][4]; // 4 : x, y, 사정거리, 충전량

			StringTokenizer stA = new StringTokenizer(br.readLine(), " ");
			StringTokenizer stB = new StringTokenizer(br.readLine(), " ");

			for (int i = 1; i <= M; i++) { // i = 0일때 값은 0으로 남아있다.(0 : 그대로)
			...
			for (int i = 0; i < bcCnt; i++) {
				...
				bc[i][2] = Integer.parseInt(st.nextToken()); // 사정거리(범위)
	...
	private static int getMaxCahrge() {
		int max = 0;
